import { clone, deserialize, fill, serialize, standardizeAny, urlEncode } from '../src/object';
...
describe('standardizeAny()', () => {
  test('return same value for simple input', () => {
    expect(standardizeAny('foo')).toEqual('foo');
    expect(standardizeAny(42)).toEqual(42);
    expect(standardizeAny(true)).toEqual(true);
    expect(standardizeAny(null)).toEqual(null);
  });

  test('return same object or arrays for referenced inputs', () => {
    expect(standardizeAny({ foo: 'bar' })).toEqual({ foo: 'bar' });
    expect(standardizeAny([42])).toEqual([42]);
  });

  test('return [undefined] string for undefined values', () => {
    expect(standardizeAny(undefined)).toEqual('[undefined]');
  });

  test('return [NaN] string for NaN values', () => {
    expect(standardizeAny(NaN)).toEqual('[NaN]');
  });

  test('iterates through array and object values to replace undefined/NaN values', () => {
    expect(standardizeAny(['foo', 42, undefined, NaN])).toEqual(['foo', 42, '[undefined]', '[NaN]']);
    expect(
      standardizeAny({
        foo: 42,
        bar: undefined,
  ...
  describe('cyclical structures', () => {
    test('must normalize circular objects', () => {
      ...
      // @ts-ignore
      obj.self = obj;
      expect(standardizeAny(obj)).toEqual({ name: 'Alice', self: '[Circular ~]' });
    });
    ...
    test('must normalize circular objects with intermediaries', () => {
      ...
      expect(standardizeAny(obj)).toEqual({ name: 'Alice', identity: { self: '[Circular ~]' } });
    });
    ...
    test('must normalize circular objects deeper', () => {
      ...
      expect(standardizeAny(obj)).toEqual({
        name: 'Alice',
    ...
    test('must normalize circular arrays', () => {
      ...
      obj.push(obj);
      obj.push(obj);
      expect(standardizeAny(obj)).toEqual(['[Circular ~]', '[Circular ~]']);
    });
    ...
    test('must normalize repeated objects in arrays', () => {
      ...
      expect(standardizeAny(obj)).toEqual([{ name: 'Alice' }, { name: 'Alice' }]);
    });
    ...
    test('must normalize error objects, including extra properties', () => {
      ...
      const result = standardizeAny(obj);
